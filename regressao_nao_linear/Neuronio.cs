...
Aprendizagem de Máquina - Regressão Não Linear com Redes Neurais Multicamadas (Classe Neurônio)  
...
namespace RegressaoNaoLinear{
    ...
    class Neuronio{
        ...
        public double Deltak(double yt, int i, int j){
            ...
            return (yt - y) * DFuncaoAtivadora(Saida(i, j));
        }

        public double Deltaj(double yt, int i, int j, int k, int l, List<Neuronio> N){

            return N[N.Count - 1].Deltak(yt, i, l) * N[N.Count - 1].w[j] * DFuncaoAtivadora(Saida(i, k));
